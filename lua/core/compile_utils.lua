...
function M.setup_compiled_lang(opts)
  ...
  map("n", "<leader>cr", function()
    ...
    local exe_name = vim.fn.fnamemodify(output, ":t")
    vim.cmd(string.format("TermExec cmd='clear && %s && ./%s'", cmd, exe_name))
  ...
  map("n", "<leader>cx", function()
    ...
    local exe_name = vim.fn.fnamemodify(output, ":t")
    vim.cmd(string.format("TermExec cmd='clear && ./%s'", exe_name))
  ...
  map("n", "<leader>cv", function()
    local _, output = get_paths()
    local exe_name = vim.fn.fnamemodify(output, ":t")
    vim.cmd(string.format("TermExec cmd='clear && valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes ./%s'", exe_name))
  end, vim.tbl_extend("force", keymap_opts, { desc = opts.name .. ": valgrind" }))

  map("n", "<leader>cg", function()
    local _, output = get_paths()
    local exe_name = vim.fn.fnamemodify(output, ":t")
    vim.cmd(string.format("TermExec cmd='clear && gdb ./%s'", exe_name))
